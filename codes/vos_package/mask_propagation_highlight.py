...
def evaluate_templates(min_similarity, search_patch, scale, best_score, best_box,
    ...
    def match_one(args):
        ...
        if H < h or W < w:
            return None
        result = cv2.matchTemplate(search_patch, template_img, cv2.TM_CCOEFF_NORMED, mask=template_mask)
...
def setup_video_writer(output_path, output_path_mask, frame_shape, fps):
    """
    Initialize a video writer to save processed frames.

    Args:
        output_path (str): File path to save the output video.
        output_path_mask (str): File path to save the output video with highlighted mask.
    ...
    out2 = cv2.VideoWriter(output_path_mask, fourcc, fps, (frame_w, frame_h))
    print(f"\n💾 Output video will be saved to: {output_path}\n")
    print(f"\n💾 Output video with highlighted mask will be saved to: {output_path_mask}\n")
    return out, out2
...
def dynamic_template_matching(video_path, initial_template,
    ...
    Args:
        ...
        output_mask_path (str): Path to save the output video with highlighted mask.
    ...
    Returns:
        None
    """
    padding, max_padding, min_padding, padding_step = 30, 50, 10, 2
    ...
    out, out2 = setup_video_writer(output_path, output_mask_path, frame.shape, fps)
    ...
    while True:
        ...
        frame_highlighted = highlight_foreground(frame, full_mask)
        out2.write(frame_highlighted)
    ...
    out.release()
    out2.release()
    print(f"[dynamic_template_matching] 🎥 Video saved successfully to {output_path}.")
    print(f"[dynamic_template_matching] 🎥 Video saved successfully to {output_mask_path}")
...
if __name__ == "__main__":
    ...
    video_path = os.path.abspath(os.path.join(base_dir, "../../data/personal/videos/clay.mp4"))
    template_path = os.path.abspath(os.path.join(base_dir, "../../outputs/personal/pre-tests/segmented/clay_segmented.png"))
    frame1_path = os.path.abspath(os.path.join(base_dir, "../../data/personal/images/clay.png"))
    output_path = os.path.abspath(os.path.join(base_dir, "../../outputs/personal/blurred/blurred_clay.mp4"))
    output_mask_path = os.path.abspath(os.path.join(base_dir, "../../outputs/personal/highlighted/highlight_clay.mp4"))
